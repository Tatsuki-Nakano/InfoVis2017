function Isosurfaces( volume, iso, screen )
{
	...
	vertexColors: THREE.VertexColors,
	    ...
	    //isovalue: { type: 'i', value: guiControls.isovalue}
        }
    });
   
    var smin = volume.min_value;
    var smax = volume.max_value;
    iso = KVS.Clamp( iso, smin, smax );
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    var v01 = interpolated_vertex( v0, v1, iso );
                    var v23 = interpolated_vertex( v2, v3, iso );
                    var v45 = interpolated_vertex( v4, v5, iso );
...
//    material.color = new THREE.Color( "white" );
    material.color = new THREE.Color().setHex( cmap[ iso ][1] );
